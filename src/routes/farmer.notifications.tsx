...
import { FARMER_NOTIFICATIONS } from "@/lib/seed";
...
function Notifs() {
  ...
  const list = tab === "All" ? FARMER_NOTIFICATIONS : FARMER_NOTIFICATIONS.filter((n) => n.kind === tab);
  return (
          ...
          return (
            <div key={n.id} className={`rounded-xl border border-[#E2E8F0] p-4 flex items-start gap-3 ${n.unread ? "bg-white border-l-4 border-l-[#2E7D32]" : "bg-[#F8FAFC]"}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-sm text-[#64748B] mt-0.5">{n.desc}</div>
              </div>
