...
function Wallet() {
  ...
  return (
    <div className="space-y-6">
      ...
      {open && uid && <WithdrawModal farmerId={uid} phone={profile?.phone ?? ""} maxAmount={balance} onClose={() => setOpen(false)} />}
    </div>
  );
}
...
function WithdrawModal({ farmerId, phone, maxAmount, onClose }: { farmerId: string; phone: string; maxAmount: number; onClose: () => void }) {
  ...
  const m = useMutation({
    mutationFn: async (amount: number) => {
      const { error } = await supabase.from("wallet_transactions").insert({
        farmer_id: farmerId,
        description: `${provider} MoMo withdrawal`,
        amount,
        type: "debit",
        status: "completed",
      });
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["wallet-tx", farmerId] }); onClose(); },
  });
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 z-50" onClick={onClose}>
      <form onSubmit={submit} onClick={(e) => e.stopPropagation()} className="w-full max-w-md bg-white rounded-2xl border border-[#E2E8F0] p-6">
        ...
        <label className="block mb-3"><span className="block text-sm font-medium mb-1.5">Phone number</span><input name="phone" className="input" defaultValue={phone} /></label>
